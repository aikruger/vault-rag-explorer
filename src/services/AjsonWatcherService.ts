...
export class AjsonWatcherService {
  ...
  private async handleRename(file: TFile, oldPath: string) {
    ...
    // We'll update the source path based on the vault-level rename.
    // Sources table stores the md file path. If the renamed file is a .md file,
    // it corresponds to a source.
    if (!file.path.toLowerCase().endsWith(".md") && !file.path.toLowerCase().endsWith(".canvas") && !file.path.toLowerCase().endsWith(".ajson")) {
      console.log("[AjsonWatcherService] ignoring rename for unindexed file type", {
        oldPath,
        newPath: file.path,
      });
      return;
    ...
    // If it's ajson, it's generated by Smart Connections.
    // If it's a markdown file, we update its path in the database.
    try {
      if (file.path.toLowerCase().endsWith(".md") || file.path.toLowerCase().endsWith(".canvas")) {
          await this.db.enqueueWrite(() => {
              this.db.updateSourcePath(oldPath, file.path);
          });
          console.log("[AjsonWatcherService] path updated in DB", {
              oldPath,
              newPath: file.path,
          });

          if ((this.indexBuilder as any).updatePathForFile) {
              (this.indexBuilder as any).updatePathForFile(oldPath, file.path);
          }

          // Generate corresponding .ajson path and schedule reindex of it
          const path = require("path");
          const ajsonFileName = file.path.replace(/\//g, "#").replace(/\.md$/, ".md.ajson").replace(/\.canvas$/, ".canvas.ajson");
          const ajsonPath = path.join(this.watchPath, ajsonFileName);
          this.scheduleReindex(ajsonPath);
      } else if (file.path.toLowerCase().endsWith(".ajson")) {
          // It's the .ajson itself getting renamed directly in vault (rare but possible)
          const fs = require("fs");
          const path = require("path");
          const fullPath = path.join((this.plugin.app.vault.adapter as any).getBasePath(), file.path);
          this.scheduleReindex(fullPath);
      }
    } catch (error) {
      console.error("[AjsonWatcherService] rename handling failed", {
        oldPath,
        newPath: file.path,
        error,
      });
